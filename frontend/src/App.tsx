...
const App: React.FC = () => {
  ...
  // Fetch chapter options whenever grade changes
  useEffect(() => {
    const loadChapters = async () => {
      try {
        const chapters = await fetchChapters(grade);
  ...
  const handleGenerate = async () => {
    ...
    // chapter as comma-separated string for existing SSE backend endpoint
    const chapterParam = selectedChapters.join(", ");
      ...
      try {
        ...
        if (data.output) {
          setOutput(
            ...
            }, Chapters: ${selectedChapters.join(", ") || "N/A"}, Difficulty: ${difficulty}${
        ...
        >
          ...
          <div className="input-row">
            <label style={{ width: "100%" }}>
              Chapters (Ctrl/Cmd + click to select multiple):
              <select
                multiple
              ...
              >
                {chapterOptions.length === 0 ? (
                  <option disabled>No chapters found for this grade</option>
                ) : (
            ...
            <div className="input-row">
              <label>
                ...
                <input
                  ...
                  value={maxMarks}
                  onChange={(e) =>
                    setMaxMarks(e.target.value === "" ? "" : Number(e.target.value))
                  }
          ...
          <button
            ...
            disabled={
              ...
              generating ||
              (isQuestionPaper &&
                (!maxMarks || isNaN(Number(maxMarks)) || Number(maxMarks) < 1)) ||
              (needsStream && !stream)
            }
          >
            {generating ? (
              ...
              <>
                <div className={`generate-slider-filler-text ${theme}-theme`}>
                  {progress === 100 && output ? (
                    ...
                    <>
                      {getFillerText(progress)}{" "}
                      <span style={{ color: "#B6901B" }}>({Math.round(progress)}%)</span>
                    </>
                  )}
                </div>
                {generating && (
